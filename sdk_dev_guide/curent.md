...
<p>
  ...
  for custom events, user profiles, crash reporting, and all other features that
  come with Countly in general, we suggest you use the
...
<p>
  <span style="font-weight:400">If this configuration is set, the SDK should not send any data to the server without consent. Even if specific SDK methods (reporting errors, recording custom events, etc.) are manually called, these calls should be ignored until consent is given.</span>
</p>
<p>
...
<ul>
  ...
  <li>
    <code>events</code> -
    <span style="font-weight:400">allow custom events to be sent to the server (doesn't apply to other features using event mechanisms)</span>
  </li>
  <li>
