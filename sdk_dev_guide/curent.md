...
<p>
  ...
  a "clear stored device ID" flag is used.<span style="font-weight: 400;"></span>
</p>
<p>
...
<p>
  <span style="font-weight: 400;">This change can be done with either with a server side merge or without it.</span>
</p>
<p>
  <span style="font-weight: 400;"><strong>Note:</strong>&nbsp;If a new and current device ID is exactly the same, then the Countly SDK must ignore this change call.</span>
</p>
<p>
...
<p>
  <span style="font-weight: 400;">It should replace the internally used device ID with the new one, and use it for all new requests, persistently storing it for further sessions. The Countly SDK should follow these steps:</span>
</p>
<ul>
  <li>
...
<p>
  <span style="font-weight: 400;">Developers may need to change a device ID to their own internal user ID and merge the server-side data previously generated by a user while he/she was unauthenticated. It is similar to "Changing ID without merging", but the Countly SDK will need to merge the data on the server as well. In order to make a proper transition, the Countly SDK should follow these steps:</span>
</p>
<ul>
  <li>
...
<p>
  ...
  is interlinked in many ways, so both will be mentioned here.
</p>
<p>
...
<p>
  But also from the SDK side, a second thing to mention is that it can send requests
  to enroll/remove users to/from A/B tests. This would affect tests running on
  the Server.
</p>
<p>
  Below Fetching Values refers to fetching values from the Countly server through
  a request generated by the SDK. Getting Values refers to getting the locally
  cached values from the device. Triggers and Flows mention how other features
  of the SDK interact with this feature.
</p>
<h2>Fetching Values</h2>
<p>
  Both RC and AB depend on fetching values. For RC, this entails fetching 'keys.'
  For AB, this consists of fetching 'variants.'
</p>
<h3>Helpers</h3>
<p>
...
<p>
  A callback is called when RC values are downloaded. Its return values:
</p>
<ul>
  <li>Result/error: Enum</li>
  <li>Error message: String</li>
  <li>
    Fully Updated?: Boolean ("true" - all values updated, "false" - a subset
    of values updated)
  </li>
  <li>
...
<p>
  A callback is called when AB variants are downloaded. Its return values:
</p>
<ul>
  <li>Result/error: Enum</li>
  <li>Error message: String</li>
...
<p>RCValue Class to deal with downloaded RC values metadata:</p>
...
<p>
  This can be automated or manually triggered by the developer.
</p>
<p>Main API:</p>
...
<p>
  Optional params 'keys' and 'omit_keys' are explained below in the Manual section.
</p>
<p>
  Optional param 'oi' is explained below in the Test Participation section.
</p>
<p>
  This would still work (some RC would be returned from the Server) without metrics,
  but it is crucial for Server to serve the correct information, so it is not optional.
</p>
<h4>Automatic</h4>
<p>RC value fetching and related processes can be automated.</p>
<p>
  This automation would mainly depend on the init time configuration:
</p>
<p>
  Config flag enables RC values to be downloaded, cached, or cleared at SDK init
  and whenever a trigger/condition is matched and disabled by default. For triggers,
  this flag should affect; please look at Triggers and Flows section. This is an
  init time configuration only.
</p>
...
<p>
  This can be called multiple times, and latest callback is used.
</p>
...
<p>
  ...
  must be updated. More information at Triggers and Flows section.
</p>
<pre><code>config.enableRemoteConfigValueCaching()</code></pre>
<h4>Manual</h4>
<p>These would be post-init calls:</p>
<p>
  Fetching has three modes: fetch all values, fetch given values, or fetch except
  given values:
</p>
<p>Fetch all values:</p>
<p>
  Makes the main API call without optional parameters and caches the downloaded
  values:
</p>
<pre><code>Countly.RemoteConfigDownloadValues(RCDownloadCallback callback)</code></pre>
<p>Fetch given values:</p>
<p>
  It makes the main API call with the optional parameter 'keys.' The developer
  should provide these keys as String Array. Then these values must be passed as
  params to the request URL (&amp;keys=["key1", "key2"...])
</p>
<pre><code>Countly.RemoteConfigDownloadSpecificValue(String[] keys, RCDownloadCallback callback)</code></pre>
<p>Fetch except for given values:</p>
<p>
  Makes the main API call with the optional parameter 'omit_keys.' The developer
  should provide these keys as String Array. Then these values must be passed as
  params to the request URL (&amp;omit_keys=["key1", "key2"...])
</p>
<pre><code>Countly.RemoteConfigDownloadOmittingValues(String[] omitKeys, RCDownloadCallback callback)</code></pre>
<p>
  Another thing the dev can manipulate manually is the callback functions for fetch
  operations.
</p>
...
<h3>A/B</h3>
<p>
  SDK fetches all or a partial amount of variants from the Server.
</p>
<p>Downloaded values are saved to memory only.</p>
<p>The main API:</p>
...
<p>
  The call for fetching variants should be something like this:
</p>
...
<p>
  The dev should be able to get the values that the SDK has fetched.
</p>
<p>
  Values can be in persistent (like RC) or volatile (like AB) storage.
</p>
<h3>RC</h3>
<p>
  RC values cached into persistent storage should get in the following fashion.
</p>
<h4>Get All</h4>
<p>Gets the map that contains all values from the storage:</p>
<pre><code>Countly.RemoteConfigGetAllValues()</code></pre>
<h4>Get Single</h4>
<p>
  Gets values for a specific key from the map of all values downloaded:
</p>
<pre><code>Countly.RemoteConfigGetValue(String key)</code></pre>
<h3>A/B</h3>
<p>
  RC variants in memory should be served in the following fashion.
</p>
<h4>Get All</h4>
<p>Returns the parsed Map&lt;String, String[]&gt; back:</p>
<pre><code>Countly.RemoteConfig.TestingGetAllVariants()</code></pre>
<h4>Get Single</h4>
<p>
  Returns a String[] for the given key. Returns null if the key does not exist.
</p>
...
<p>
  A/B tests enroll users into experiments according to some logic at the server
  side. However, a dev can affect that decision from the SDK by explicitly enrolling
  or removing a user from (a) test(s).
</p>
<h3>Enrolling Users</h3>
<p>
  There are 2 APIs to enroll a user into a test. There used to be a third one,
  as mentioned above (oi), but it is deprecated now.
</p>
<p>
...
<pre><code>Countly.RemoteConfig.TestingEnrollIntoVariant(String keyName, String variantName, RCVariantCallback completionCallback)</code></pre>
<p>
  Or a user can be enrolled to some keys (["key1","key2"...]) with this API:
</p>
...
<p>Dev should provide an array of string keys for that:</p>
<pre><code>Countly.RemoteConfigEnrollIntoABTestsForKeys(String[] keys)</code></pre>
...
<p>
  Dev should provide an array of string keys, or else the user would be removed
  from all tests:
</p>
<pre><code>Countly.RemoteConfigExitABTestsForKeys(String[] keys)</code></pre>
<h2>Triggers and Flows</h2>
<p>
...
<h3>SDK Init</h3>
<p>We download the RC values at init.</p>
<h3>Temp ID</h3>
<p>Entering temp ID mode should clear the RC cache.</p>
<p>Coming out of temp ID mode should download RC values again.</p>
<h3>Consent</h3>
<p>
  Receiving "remote-config" consent should trigger RC values to download.
</p>
<p>Removing that consent should clear the stored values.</p>
<h3>Device ID Change</h3>
<p>
  Changing the device ID without merge should clear cached RC values.
</p>
<p>And re-download them for the new ID.</p>
<h1>User feedback</h1>
<h2>Star Rating</h2>
<p>
...
<p>
  ...
  <code>getAvailableFeedbackWidgets</code>. That call takes a callback. That callback
  returns 2 values. The second is the error string. The first one a list of available
  widget objects (or any other mechanism that allows the grouping of this data).
...
<p>
  If temporary device ID is enabled, feedback widgets can't be shown so an empty
...
<ol>
  <li>
    ...
    initial step with the automatic feedback widgets and reuses the same call
    to retrieve them.
  </li>
  <li>
...
<p>
  Second step uses the <code>CountlyFeedbackWidget</code> object from the previous
...
<p>
  ...
  forcefully combined into a request, event if the event count is under the threshold.
...
<pre class="c-mrkdwn__pre" data-stringify-type="pre">{<br> "v":1,<br> "t":1681808287464,<br> "c":{<br>  "tracking":false,<br>  "networking":false,<br>  "crashes":false,<br>  "views":false,<br>  "heartbeat":61,<br>  "event_queue":11,<br>  "request_queue":1001<br> }<br>}</pre>
<div class="p-rich_text_section">
  <span class="c-mrkdwn__br" aria-label="" data-stringify-type="paragraph-break"><strong>v</strong> - current schema version</span>
</div>
<div class="p-rich_text_section">
  <span class="c-mrkdwn__br" aria-label="" data-stringify-type="paragraph-break"><strong>t</strong> - timestamp</span>
</div>
<div class="p-rich_text_section">
  <span class="c-mrkdwn__br" aria-label="" data-stringify-type="paragraph-break"><strong>c</strong> - config object</span>
</div>
<div class="p-rich_text_section">
...
<div class="p-rich_text_section">
  <span class="c-mrkdwn__br" aria-label="" data-stringify-type="paragraph-break"></span>Server
  has a default values for every field.<br>
  If the currently set value differs from the default, it will be sent by the server.
  If no value is sent for a configuration then that means that the SDK has to use
  it's own default or the value provided by the developer.
</div>
<div class="p-rich_text_section">
  <span class="c-mrkdwn__br" aria-label="" data-stringify-type="paragraph-break"></span>
</div>
<div class="p-rich_text_section">
  <span class="c-mrkdwn__br" aria-label="" data-stringify-type="paragraph-break"></span><span class="c-mrkdwn__br" aria-label="" data-stringify-type="paragraph-break"></span>SDK
  side the precedence of configs will be:
</div>
<pre class="c-mrkdwn__pre" data-stringify-type="pre">SDK default &lt; Dev set value &lt; server config value</pre>
<p>
  Once per init the SDK tries to acquire the config and stores it persistently
  locally.
</p>
<p>
